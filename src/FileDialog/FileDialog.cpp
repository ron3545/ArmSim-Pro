...
namespace ArmSimPro {
	...
	void FileDialog::m_renderFileDialog()
	{
		...
		/***** CONTENT *****/
		float bottomBarHeight = ((GImGui->FontSize + ImGui::GetStyle().FramePadding.y + ImGui::GetStyle().ItemSpacing.y * 2.0f) * 2) + 8;
		...
		if (ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) &&
			 escapeKey >= 0 && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Escape)))
			m_isOpen = false;
	}
}
...
const char* ArmSimPro::GetDefaultFolderIcon()
{
	return (const char*)&folder_icon[0];
}
const char* ArmSimPro::GetDefaultFileIcon()
{
